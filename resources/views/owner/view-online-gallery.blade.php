                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="7" class="text-center py-4">
                                                ...
                                                No in-progress or completed bookings with online gallery found
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.create-gallery, .manage-gallery', function() {
                ...
                $.ajax({
                    url: `{{ url('owner/online-gallery') }}/${bookingId}/details`,
                    type: 'GET',
            ...
            $('#uploadGalleryForm').on('submit', function(e) {
                ...
                $.ajax({
                    url: `{{ url('owner/online-gallery') }}/${selectedBookingId}/upload`,
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
            ...
            $(document).on('click', '.delete-image', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: `{{ url('owner/online-gallery') }}/${currentGalleryId}/image`,
                            type: 'DELETE',
                            data: {
                                _token: '{{ csrf_token() }}',
            ...
            $('#deleteGalleryBtn').on('click', function() {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: `{{ url('owner/online-gallery') }}/${currentGalleryId}`,
                            type: 'DELETE',
                            data: {
                                _token: '{{ csrf_token() }}'
                            },

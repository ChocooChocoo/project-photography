...
> **In plain terms:** This is the full analysis of trial, renewal, expiry, and reactivation decisions. It is not an approved implementation plan.
>
...
[`ROADMAP PROGRESS.md`](../05-progress/delivery-history.md). Four defects, all verified
against source:
...
[`ROADMAP PROGRESS.md`](../05-progress/delivery-history.md). A trial length set at
...
| Where | Says | Actually |
|---|---|---|
| [`TECHNICAL ANALYSIS.md`](../02-analysis/technical-analysis.md) §5.9 flowchart | `Owner registers + creates studio → {Active subscription? CheckStudioRegistrationLimit} → no: Block create` | The **first** studio needs no subscription. The gate fires only from the second onward, only on POST, and not at all for `owner-super-admin`. |
| [`NON TECHNICAL ANALYSIS.md`](../00-overview/plain-summary.md) §4.7 | *"They need an active subscription."* | Same correction, in the prose and the mermaid diagram. |
| [`CAPSTONE B IMPLEMENTATION ROADMAP.md`](../05-roadmap/roadmap.md) 6.4 | *"On expiry: downgrade to free tier"* | **No free tier exists.** `plan_type` is `basic\|premium\|enterprise` and all eight seeded plans are priced (₱590–₱27,900). There is nothing to downgrade to. |

A fourth is stale rather than wrong:
[`REVISION CHECKLIST AND RECOMMENDATIONS.md`](../02-analysis/revision-checklist.md)
...
- [`02-PLANNING/CAPSTONE B IMPLEMENTATION ROADMAP.md`](../05-roadmap/roadmap.md) — Phase 10, and the 3.4 / 6.4 cross-references.
- [`03-PROGRESS/ROADMAP PROGRESS.md`](../05-progress/delivery-history.md) — Phase 10 status table, and the 3.4 record of what shipped.
- [`01-ANALYSIS/REVISION CHECKLIST AND RECOMMENDATIONS.md`](../02-analysis/revision-checklist.md) — Part 4 item on the lifecycle, and item 11 on the free trial.
